...
class aliexpressSpider(Spider):
    ...
    def parse(self, response):
        ...
        if self.category_index >= len(main_cats_tags):
            self.category_index = 0

        main_cats_tag = main_cats_tags[self.category_index]
        ...
        # response.meta['proxy'] = 'socks5://proxy.dnsflex.com:51317'

        yield Request(url=url_for_sub_menu,
    ...
    def parse_sub_menu(self, response):
        if 'https://login.' in response.url:
            ...
            self.err_parse(response)
            return
        sub_cat_items = response.xpath('//dl[@class="sub-cate-items"]/dd/a')
        for sub_cat_item in sub_cat_items:
            ...
            # response.meta['proxy'] = 'socks5://proxy.dnsflex.com:51317'
            # resp = requests.get(response.urljoin(url_for_sub), proxies=dict(http='gcm:Money102!@107.172.238.174:80'))

            # cat_url = 'https://www.aliexpress.com/glosearch/api/product' \
            #           '?trafficChannel=main' \
            #           '&catName={}' \
            #           '&CatId={}' \
            #           '&ltype=wholesale' \
            ...
            #           '&page=1' \
            #           '&isrefine=y' \
            #           '&origin=y'.format(sub_menu_title.lower(), cat_id)

            yield Request(url=response.urljoin(url_for_sub),
    ...
    def parse_product_detail(self, response):
        ...
        diff = end_second_of_product - self.start_second
