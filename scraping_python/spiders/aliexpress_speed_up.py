...
class aliexpressSpider(Spider):
    ...
    def start_requests(self):
        yield Request(url=self.start_url,
                      callback=self.parse,
                      meta={'method_string': 'parse', 'proxy': 'gcm:Money102!@138.94.217.121:80',
                            'url': self.start_url}
                      )
                            ...
                            for item in items:
                                ...
                                while True:
                                    ...
                                    if ('https://login.' in resp_product_temp.url) or ('location.href="https://login.aliexpress.com/' in html_product):
                                        self.logger.error('Error: login page found to get product data.')
                                        time.sleep(5)
                                    else:
                                        break
                                ...
                                yield Request(url_for_shipping,
    ...
    def parse_shipping(self, response):
        ...
        sellerAdminSeq = response.meta['sellerAdminSeq']

        shipping_method_data = json.loads(response.text)
        if shipping_method_data:
            body = shipping_method_data.get('body')
            freightResult = body.get('freightResult')
            for i, freightRe in enumerate(freightResult):
        ...
        url_for_store_feedback = 'https://feedback.aliexpress.com//display/evaluationDetail.htm' \
                                                         ...
                                                         '&callType=iframe' \
                                                         '&iframe_delete=true'.format(str(sellerAdminSeq))
        yield Request(url_for_store_feedback, callback=self.parse_feedback, meta=response.meta)
    ...
    def parse_feedback(self, response):
        ...
        #                     body=resp_temp.text,
        #                     encoding='utf-8')
        feedback_dsr = response.xpath('//div[@id="feedback-dsr"]//table/tbody/tr')
        for feedback in feedback_dsr:
            th_val = feedback.xpath('./th/text()').extract_first()
            val = feedback.xpath('./td/span[@class="dsr-text"]/em/text()').extract_first()
            if 'Item as Described' in th_val:
        ...
        feedback_history = response.xpath('//div[@id="feedback-history"]//table/tbody/tr')
        feedback_titles = []
        for feedback in feedback_history:
            th_val = feedback.xpath('./th/text()').extract_first()
            if 'Feedback' in th_val:
