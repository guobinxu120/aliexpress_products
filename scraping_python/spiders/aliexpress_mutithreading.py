    ...
    def parse(self, response):
        ...
        while True:
            ...
            for main_cats_tag in main_cats_tags:
                ...
                resp_main_temp = requests.get(url_for_sub_menu)
                ...
                for sub_cat_item in sub_cat_items:
                    ...
                    self.logger.info('The category - {} of {} started to scrape from {}'.format(sub_menu_title, main_menu_title, str(start_second)))
                    ...
                    while True:
                        ...
                        while True:
                            ...
                            # length = resp_sub_temp.headers['content-length']
                            html = resp_sub_temp.read(int(8092))

                            if ('https://login.' in resp_sub_temp.url) or ('location.href="https://login.aliexpress.com/' in html):
                                self.logger.error('Error: login page found to get product list.')
                                time.sleep(5)
                                socks.set_default_proxy(socks.SOCKS5, self.SOCKET5_PROXY_HOST, self.SOCKET5_PROXY_PORT)
                                socket.socket = socks.socksocket
                            else:
                        ...
                        if products_data:
                            ...
                            queue = Queue()
                            for x in range(len(product_detail_url_list)):
                                worker = DownloadWorker(queue)
                                # Setting daemon to True will let the main thread exit even though the workers are blocking
                                worker.daemon = True
                                worker.start()
                            # Put the tasks into the queue as a tuple
                            for link in product_detail_url_list:
                                if 'https' not in link:
                                    link = 'https:' + link
                                self.logger.info('Queueing {}'.format(link))
                                queue.put((response, link))
...
class DownloadWorker(Thread):
    ...
    hdlr = logging.FileHandler('log_for_DownloadWorker.log')
    formatter = logging.Formatter('%(asctime)s - %(name)s %(levelname)s %(funcName)s() %(lineno)d:\t %(message)s')
    hdlr.setFormatter(formatter)
    logger.addHandler(hdlr)
    logger.setLevel(logging.INFO)
    ...
    def run(self):
        while True:
            ...
            response, product_detail_url = self.queue.get()
            try:
                self.get_product_data(response, product_detail_url)
            finally:
                self.queue.task_done()

    def get_product_data(self, response, product_detail_url):
        # get product data
        html_product = ''
        while True:
            resp_product_temp = urllib2.urlopen(response.urljoin(product_detail_url))
            # length = resp_product_temp.headers['content-length']
            # print(length)
            html_product = resp_product_temp.read(int(8092))
